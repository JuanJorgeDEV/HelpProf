...
Seja objetiva, empática e use passos curtos. Não invente funcionalidades obscuras; prefira o que está nas pílulas ou em fontes oficiais encontradas na busca.
"""
...
def _build_pill_prompt(row: dict, question: str) -> str:
    blocks = [
        _PILL_SYSTEM,
        (
            f"## Pílula atual\n"
...
def run_lume(payload: LumeQueryIn) -> LumeQueryOut:
    ...
    if payload.scenario == "pill" and payload.pill_id:
        ...
        prompt = _build_pill_prompt(row, q)
